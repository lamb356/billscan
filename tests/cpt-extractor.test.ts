...
describe('cpt-extractor', () => {
  describe('5-digit numeric CPT code extraction', () => {
    it('extracts a single 5-digit CPT code with dollar amount', async () => {
      const text = '99285 Emergency department visit level 5 $2,847.00';
      const result = await extractCptData(text);
    ...
    it('extracts dollar amount correctly for 5-digit code', async () => {
    ...
    it('extracts multiple 5-digit CPT codes from multi-line text', async () => {
      const text = [
    ...
    it('does not extract obviously invalid 5-digit numbers (zip codes in address)', async () => {
  ...
  describe('J-code extraction', () => {
    it('extracts J-codes (J + 4 digits)', async () => {
    ...
    it('extracts J-code billing amount', async () => {
    ...
    it('extracts J-codes alongside 5-digit CPT codes', async () => {
      const text = [
  ...
  describe('dollar amount extraction', () => {
    it('extracts $ prefixed amounts', async () => {
    ...
    it('extracts comma-formatted amounts', async () => {
    ...
    it('extracts bare decimal amounts (no dollar sign)', async () => {
    ...
    it('skips codes with no associated amount', async () => {
  ...
  describe('line association (CPT + amount on same line)', () => {
    it('associates a CPT code with an amount when it is on its own line', async () => {
    ...
    it('extracts both CPT codes when given two single-code lines (lookahead behavior)', async () => {
    ...
    it('preserves rawLine on each extracted item', async () => {
      const text = '99285 Emergency visit $2,847.00';
      const result = await extractCptData(text);
      const item = result.lineItems.find(i => i.cptCode === '99285');
      assert.ok(item);
  ...
  describe('totalBilled calculation', () => {
    it('sums all extracted line item amounts', async () => {
    ...
    it('sums amounts across multiple separate single-code calls correctly', async () => {
      const r1 = await extractCptData('99285 ED visit $2,847.00');
      const r2 = await extractCptData('71046 Chest X-ray $847.00');
      const combined = r1.totalBilled + r2.totalBilled;
      assert.strictEqual(combined, 2847 + 847);
    });

    it('returns 0 total when no line items extracted', async () => {
  ...
  describe('confidence passthrough', () => {
    it('uses provided ocrConfidence value', async () => {
      const result = await extractCptData('99285 ED visit $500.00', 0.92);
      assert.strictEqual(result.confidence, 0.92);
    });

    it('defaults to 0.85 confidence', async () => {
      const result = await extractCptData('99285 ED visit $500.00');
