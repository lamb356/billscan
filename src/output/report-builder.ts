...
export function formatReportConsole(report: AuditReport): string {
  ...
  lines.push(`║  Generated: ${report.stamp.generatedAt.padEnd(68)}║`);
  lines.push(`║  Report ID: ${report.stamp.reportId.padEnd(68)}║`);
  const sources = report.stamp.dataSources?.join('+') || 'PFS';
  lines.push(`║  CMS Data: ${report.stamp.cmsEffectiveYear} [${sources}] (hash: ${report.stamp.cmsDataHash.slice(0, 16)}...)${' '.repeat(Math.max(0, 47 - sources.length))}║`);
  lines.push('╚══════════════════════════════════════════════════════════════════════════════════════╝');
  lines.push('');
  ...
  for (const f of report.findings) {
    ...
    lines.push(`│ ${String(f.lineNumber).padStart(3)} │ ${f.cptCode.padEnd(6)} │ ${desc} │ ${billed} │ ${fac} │ ${nfac} │ ${delta} │ ${source} │ ${match} │`);
  }
...
// ─── EOB / Insurance Comparison Report ───────────────────────────────────────

/**
 * Formats an EOB audit report to the console.
 * Shows standard CMS columns PLUS an insurance comparison column.
 * When --plan is used, the column shows estimated allowed range.
 * When --eob is used, the column shows actual allowed amount from the EOB.
 */
export function formatEobAuditConsole(eobReport: EOBAuditReport): string {
  ...
  // ── Table ──
  //  #  | CPT    | Description              | Billed   | CMS Rate  | Allowed/Est       | vs CMS   | vs Allowed
  lines.push('┌─────┬────────┬──────────────────────────┬──────────┬───────────┬───────────────────┬──────────┬────────────┐');
  lines.push(`│  #  │ CPT    │ Description              │  Billed  │ CMS Rate  │ ${allowedColHeader.padEnd(17)} │  vs CMS  │ ${vsAllowedHeader.padEnd(10)} │`);
  lines.push('├─────┼────────┼──────────────────────────┼──────────┼───────────┼───────────────────┼──────────┼────────────┤');
  ...
  for (let i = 0; i < baseReport.findings.length; i++) {
    ...
    lines.push(`│ ${String(f.lineNumber).padStart(3)} │ ${f.cptCode.padEnd(6)} │ ${desc} │ ${billed} │ ${cms} │ ${allowedCol} │ ${vsCms} │ ${vsAllowedCol} │`);
  }
  ...
  if (eobReport.insights.length > 0) {
    ...
    const ICONS: Record<string, string> = { info: 'ℹ️ ', warning: '⚠️ ', alert: '🚨' };
