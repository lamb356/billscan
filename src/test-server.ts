...
async function main() {
  ...
  await sleep(500); // small extra wait for port to bind
  ...
  try {
    ...
    log(`Total savings: $${stats.totalSavingsFound?.toLocaleString()}`);
    log('PASS');
    ...
    log(`Contains API calls: ${html.includes('/api/')}`);
    ...
    if (report.error) {
      ...
      log(`CMS Baseline: $${report.totalCmsBaseline?.toLocaleString()}`);
      log(`Potential Savings: $${report.totalPotentialSavings?.toLocaleString()}`);
      log(`Matched: ${report.matchedLineCount}, Unmatched: ${report.unmatchedLineCount}`);
      log(`Avg Multiplier: ${report.summary?.averageMultiplier}x`);
      log(`Data Sources: ${report.stamp?.dataSources?.join(', ')}`);
      log('\nTop findings:');
      report.findings?.slice(0, 5).forEach((f: any) => {
        log(`  [${f.cptCode}] ${f.description} | billed=$${f.billedAmount} cms=$${f.cmsRateUsed ?? 'N/A'} severity=${f.severity ?? 'none'} source=${f.rateSource ?? 'N/A'}`);
      });
      log('PASS');
    }

    // Test 6: Charity check
    ...
    log(`EIN: ${charity.ein}`);
    log(`Advice lines: ${charity.advice?.length}`);
    log('PASS');

    // Test 7: Audit with ZIP
    log('\n--- Test 7: POST /api/audit/json?zip=10001&setting=facility ---');
    const report2 = await post(`/api/audit/json?zip=10001&setting=facility`, JSON.parse(billJson));
    if (report2.error) {
      log(`ERROR: ${report2.error}`);
    } else {
      log(`Report ID: ${report2.stamp?.reportId}`);
      log(`Setting used: facility`);
      log(`Matched: ${report2.matchedLineCount}`);
      log('PASS');
    }
