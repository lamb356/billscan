...
import { parseBill, type ParseResult } from '../parser/bill-parser.js';
...
import type { EOBDocument } from '../parser/eob-parser.js';
...
export async function runAudit(billPath: string, options: AuditOptions = {}): Promise<AuditResult> {
  // 1. Parse bill (may also detect EOB)
  const parseResult: ParseResult = await parseBill(billPath);
  ...
  return {
    report,
    isEob: parseResult.isEob,
    eob: parseResult.eob,
    eobRaw: parseResult.eobRaw,
  };
}
