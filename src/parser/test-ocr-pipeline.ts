 ...
 * Exercises cpt-extractor.ts and bill-from-text.ts on sample text.
 *
...
const extracted = await extractCptData(billText, 0.95);
...
for (const code of EXPECTED_CODES) {
  ...
  console.log(`  ${found ? '✓' : '✗'} ${code}${found ? '' : ' — NOT FOUND'}`);
...
// ─── Test 2: bill-from-text converter ─────────────────────────────────────
...
// ─── Test 3: Inline text snippets ─────────────────────────────────────────
console.log('\nTEST 3: Inline text snippet extraction');
console.log('─'.repeat(52));

const snippets: { label: string; text: string; expectCode: string; expectAmount: number }[] = [
  {
    label: 'Simple tabular row',
    text: '99285   Emergency visit Level 5   $2,847.00',
    expectCode: '99285',
    expectAmount: 2847.00,
  },
  {
    label: 'J-code drug row',
    text: 'J3490 UN Acetaminophen 500mg $47.00',
    expectCode: 'J3490',
    expectAmount: 47.00,
  },
  {
    label: 'HCPCS alpha code',
    text: 'G0439  Annual wellness visit  $185.00',
    expectCode: 'G0439',
    expectAmount: 185.00,
  },
  {
    label: 'Amount without dollar sign',
    text: '36415   Venipuncture   189.00',
    expectCode: '36415',
    expectAmount: 189.00,
  },
  {
    label: 'Multiple amounts — pick last (billed charge)',
    text: '93000  ECG Complete  1  $1,243.00  $621.00  $622.00',
    expectCode: '93000',
    expectAmount: 622.00,
  },
];
...
for (const s of snippets) {
  const result = await extractCptData(s.text, 0.9);
  const found = result.lineItems.find(i => i.cptCode === s.expectCode);
  const codeOk = !!found;
  const amtOk = found?.billedAmount === s.expectAmount;
  const pass = codeOk && amtOk;
