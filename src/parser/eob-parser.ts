...
export interface EOBLineItem {
  ...
  adjustmentReason: string | null; // e.g. "exceeds usual and customary"
  copay: number;
  coinsurance: number;
  deductible: number;
}
...
function parseLineItem(raw: Record<string, unknown>, idx: number): EOBLineItem {
  ...
  // Validate coherence: allowedAmount ≈ insurancePaid + patientResponsibility (within $0.05 rounding)
...
/**
 * Parses an EOB JSON file and returns a validated EOBDocument.
 * Throws descriptive errors on malformed input.
 */
export function parseEob(filePath: string): EOBDocument {
  if (!existsSync(filePath)) {
    throw new Error(`EOB file not found: ${filePath}`);
  }
  ...
  // Totals: prefer explicit values, fall back to computed sums
  ...
  // Warn if supplied totals differ from computed (tolerance: $1.00 for rounding)
...
/**
 * Returns a map of cptCode → EOBLineItem for fast lookup during auditing.
 * If a code appears multiple times, the highest-allowedAmount line wins.
 */
