...
async function downloadAndExtract(
  ...
  for (const url of urls) {
    ...
    try {
      ...
      if (!response.ok) {
        console.log(`[${label}] ${response.status} from ${url}`);
        continue;
      }
      if (response.body) {
        ...
        try {
          ...
          console.log(`[${label}] Invalid ZIP from ${url}`);
          continue;
        }
...
function extractBestCsv(
  ...
  // Use the picker to find the best CSV/TXT
  const chosen = csvPicker(names);
  if (!chosen) {
    // Fallback: biggest CSV/TXT
...
export async function fetchOppsData(year: number, forceRefresh = false): Promise<FetchResult> {
  ...
  return downloadAndExtract(urls, 'opps-fetcher', downloadDir, `opps-${year}`, forceRefresh, (entries) => {
    // Prefer the 508 / CSV version of Addendum B
