...
/**
 * CMS PFREV/PFALL national payment amount files use TWO formats:
 *
 * 1. Headerless positional CSV (current as of 2025+):
 *    16 quoted columns: Year, Carrier, Locality, HCPCS, Modifier,
 *    Par_Fac, Par_NonFac, ..., Status, ...
 *
 * 2. Legacy format with header row containing 'HCPCS'
 */

// Legacy header column aliases
...
// Positional column indices for the PFALL headerless format (16 columns)
const POS = {
  year: 0,
  carrier: 1,
  locality: 2,
...
/**
 * Streaming parser that calls onBatch for every N rates parsed.
 * This avoids holding all 1M+ rates in memory.
 */
export async function parseCmsCsvStreaming(
  ...
  for await (const line of rl) {
    ...
    if (isPositional) {
      ...
      if ((facilityRate === null || facilityRate === 0) &&
          (nonFacilityRate === null || nonFacilityRate === 0)) {
        skipped++;
        continue;
      }
      rate = {
    ...
    if (lineNum % 200000 === 0) {
      console.log(`[cms-parser] ${lineNum} lines processed, ${totalParsed} rates parsed...`);
    }
  }
  ...
  if (totalParsed === 0) {
    throw new Error(`[cms-parser] No valid rates parsed from ${csvPath}.`);
  }
...
/**
 * Legacy non-streaming parser (loads everything into memory).
 * Kept for small files / backwards compatibility.
 */
export async function parseCmsCsv(csvPath: string, effectiveYear: number): Promise<ParseResult> {
  const rates: CMSRate[] = [];
  const result = await parseCmsCsvStreaming(csvPath, effectiveYear, 50000, (batch) => {
    rates.push(...batch);
  });
...
function buildHeaderMap(headers: string[]): Record<string, number> {
  ...
  for (const [field, aliases] of Object.entries(COLUMN_ALIASES)) {
    ...
    if (idx >= 0) {
      map[field] = upperHeaders.indexOf(aliases[idx]);
    } else {
      map[field] = -1;
    }
  }
...
function parseCsvLine(line: string): string[] {
  ...
  // Fast path for quoted CSV (like CMS positional format)
  const fields: string[] = [];
  let pos = 0;
