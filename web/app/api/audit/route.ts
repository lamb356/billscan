...
export const maxDuration = 60; // Vercel: allow up to 60s for audit
...
export async function POST(req: NextRequest) {
  ...
  if (!usage.allowed) {
    return NextResponse.json(
      {
        error: `Monthly limit reached. You've used all ${usage.limit} free audits this month.`,
        code: 'USAGE_LIMIT',
  ...
  try {
    ...
    const auditResult = await runAudit(tmpPath, {
      zip,
      locality,
      setting,
      save: true, // persist for shareable reports
    });
  ...
  if (isPro) {
    try {
      disputeLetter = generateDisputeLetter(report);
    } catch (err) {
      console.error('[audit] dispute letter error:', err);
    }
    try {
      phoneScript = generatePhoneScript(report);
    } catch (err) {
      console.error('[audit] phone script error:', err);
    }
  }
  ...
  return NextResponse.json({
    ...
    shareUrl: `/report/${report.stamp.reportId}`,
    usageAfter: {
      count: usage.count + 1,
      limit: usage.limit,
      isPro,
