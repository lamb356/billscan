...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Aggregate stats from the audits table
    const [totalsResult, topSavingsResult, recentResult] = await Promise.all([
      ...
      db.execute({
        sql: `
          SELECT report_id, total_billed, total_savings, finding_count, created_at
          FROM audits
    ...
    return NextResponse.json({
      ...
      topSavings: topSavingsResult.rows.map((r) => ({
      ...
      recentAudits: recentResult.rows.map((r) => ({
        reportId: r.report_id,
        totalBilled: r.total_billed,
        totalSavings: r.total_savings,
        findingCount: r.finding_count,
