...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const userResult = await db.execute({
      sql: 'SELECT id, stripe_customer_id, plan FROM users WHERE email = ?',
      args: [email],
    });
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/audit?upgraded=true`,
...
// Redirect GET requests to the checkout URL (for direct links in the pricing page)
export async function GET(req: NextRequest) {
  ...
  // POST the checkout from here for the GET flow
