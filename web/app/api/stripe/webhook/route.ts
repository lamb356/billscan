  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (email) {
          await setUserPlan(db, email, 'pro', customerId, subscriptionId);
          console.log(`[stripe/webhook] checkout.session.completed → ${email} upgraded to pro`);
        }
        break;
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        if (customerId) {
          ...
          console.log(`[stripe/webhook] subscription.deleted → customer ${customerId} downgraded`);
        }
        break;
      }
